...
const SamplePolicies: Array<PolicyCardProps> = [
    {
        ...
        content: "A society is made up of people. A health population brings about a healthy society.",
        imageUrl: health,
    },
    {
        title: "Stronger Families",
        content: "The family is the smallest unit of humanity. Whether you're in a small or large family, we wish to give all the support we can.",
        imageUrl: community,
    },
    {
        title: "Higher Education",
        content: "Since the industrial revolution, education has been the driving force for societal advancement. To support Australia's scientific and advanced industries, we are investing as much as we can into stronger education",
        imageUrl: education,
    },
    {
        title: "Stronger Borders",
        content: "Security for our citizens is paramount. We wish the best for everyone, Australians and foreigners. But not everyone should be calling Australia home, especially those willing to cause harm.",
        imageUrl: national_security,
    },
]

function Policies() {
    const [policies, setPolicies] = React.useState<Array<PolicyCardProps>>(SamplePolicies)
    ...
    const getPolicy = (prompt: string, callback: any) => {
        axios.get(policyUrl, { params: { policy: prompt } })
            .then(res => setPolicies([...policies, { title: prompt, content: res.data['text'], imageUrl: PolicyTypeMap[value] }]))
            .then(callback())
    ...
    return (
        <>
            <Banner {...{ title: "Policies", subtitle: "Understand the policies we're implementing", quote: "If the facts don't fit the theory, change the facts", author: "Albert Einstein" }} />
            <Flex flexDir="column" justifyContent="center" alignItems="center">
                ...
                <Flex flexDir="column" justifyContent="center" alignItems="center" mb="4rem">
                    <Text mb="2rem">Helpful?</Text>
                    {!helpful ? <Flex>
                                    <Button backgroundColor="black" color="white" mr="2rem" onClick={handleHelpfulClick}>Yes</Button>
                                    <Button backgroundColor="black" color="white" onClick={handleHelpfulClick}>No</Button>
                                </Flex> :
