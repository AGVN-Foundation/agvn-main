...
import { Flex, Box, Button, Grid, Heading, Radio, RadioGroup, Text, Textarea } from '@chakra-ui/react'
...
const PolicyTypes = [
    'taxation', 'lifestyle', 'community', 'infrastructure', 'foreign relations', 'health', 'education',
...
const PollPage = () => {
    ...
    const [polls, setPolls] = useState<{ id: number; question: string; ongoing:boolean; type: number; end_date: string; possible_answers: string; subject: number; }[]>([]);
        ...
        axios
            ...
            .catch(function (error) {
                if (error.response.status === 400) {
                    setErrorMsg("You have already voted, please contact admin if you want to revote.")
                }
            })
    }
    ...
    const generatePoll = () => {
        ...
        axios.get(url, { params: { policy_type: PolicyTypes[radioValue] } })
            .then(res => {
                let _data = res.data
                console.log(_data)
            ...
            </Heading>
            {
                ...
                <MDBContainer>
                    <MDBRow center between>
                        <MDBCol size='4'>
                            <Text><b>Question {index+1}</b>: {p.question}</Text>
                        </MDBCol>
                        <MDBCol md="5">
                        <Poll type={p.type} pollQuestion={p.question} pollAnswers={p.possible_answers} question_id={p.id} />
                        </MDBCol>
                        <MDBCol size='auto'>
                            <Button bg="darkestBlue" color="white" onClick={() => sendPoll(p.id)}>
                                Submit
                            </Button>
                    ...
                    <RadioGroup mt="2rem" onChange={setRadioValue}>
                        <Flex flexDir="row" p="2rem" flexWrap="wrap">
                            {Object.keys(PolicyTypes).map((key) => (
                                <Radio key={key} value={key} m="1rem">{PolicyTypes[key]}</Radio>
                            ))}
                        </Flex>
                    </RadioGroup>
    ...
    const logoutProp = (
        <Flex as="form" flexDirection="column">
            <Heading mt="10" mb="10" size="2xl" textAlign="center">Login to do polls!</Heading>
        </Flex>
    )
    ...
    return (
        <>
            {Banner({ title: "Poll", subtitle: "Boost your contribution by completing polls", quote: "We are of different opinions at different hours, but we always may be said to be at heart on the side of truth", author: "Ralph Emerson" })}
