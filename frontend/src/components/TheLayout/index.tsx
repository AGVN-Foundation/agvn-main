...
const TheLayout: React.FC<TheLayoutProps> = ({ children }) => {
  ...
  const getReply = (
    ...
    axios
      .get(chatbotUrl, { params: { message: input } })
      .then((res) => {
        ...
        setMessageHistory([
          ...messageHistory,
          userMessage,
          { by: 1, message: res.data.message, id: currId + 2 },
        ])
        currId = currId + 2
        console.log(messageHistory)
      })
  ...
  >([{ by: 1, message: "Welcome to AGVN!", id: 1 }])
          ...
          <ModalBody style={{ overflowY: "scroll" }}>
            <Flex flexDir="column" w="100%" ref={messageEvent}>
              {messageHistory.map((m) => (
                <Flex
                  marginLeft={m.by === 0 ? "auto" : ""}
                  key={m.id}
                  backgroundColor={m.by === 0 ? "#4287f5" : "#e6e6e6"}
                  color={m.by === 0 ? "#f0f0f0" : "#3d3c3c"}
                  fontWeight="semibold"
                  borderRadius="25px"
                  w="50%"
                  mb="1rem"
                >
                  <motion.div whileHover={{ scale: 1.1 }}>
                    <Flex m="1rem">
                      <Text pl="1.5rem">{m.by === 1 ? "Bot: " : "You: "}</Text>
                      <Text ml=".5rem">{m.message}</Text>
                    </Flex>
                  </motion.div>
                </Flex>
              ))}
