...
import { Text, Box, Flex, Button, Heading, Image, Divider } from "@chakra-ui/react"
...
const VotePage: React.FC<VotePageProps> = (props) => {
  ...
  const [items, setItems] = useState<{ type: string; icon: any; value: number }[]>([]);
  const [selected, setSelected] = useState<{ type: string; icon: any; value: number }[]>([]);
  ...
  const voteProp = (
    <>
      <Flex className="vote-container" justifyContent="center" alignItems="center" flexDir="column" mb="5rem">
        <Heading textAlign="center" mb="1rem">Vote for Initiatives</Heading>
      ...
      <Flex>
        <Flex flexDir="column">
          <Text textAlign="center" mb="1rem" fontWeight="semibold" color="#1d2533" fontSize="1.5rem">Choose from</Text>
          <Box minH="250px" w="25rem" border="1px solid gray">
            {items
              .filter((item) => !selected.some(x => x === item))
              .map(item => (
                <Flex key={item.type} justify="center">
                  <Button w="95%" my="8px" onClick={() => {setSelected([...selected, item]); setErrorMsg('')}}>
        ...
        <Flex justify="center" alignItems="center" m="2rem">{">"}</Flex>
        <Flex flexDir="column">
          <Text textAlign="center" mb="1rem" fontWeight="semibold" color="#1d2533" fontSize="1.5rem">Your choices</Text>
          <Box minH="250px" minW="25rem" border="1px solid gray">
            {selected
              .map(item => {
                if (item.value !== 0){
                  return <Flex key={item.value} justify="center">
                    <Button w="95%" my="8px" onClick={() => {setSelected([...selected.filter(x => x !== item)]); setErrorMsg('')}}>
                      {item.type}
                      <Image src={item.icon} height="2rem" ml="0.5rem" />
                    </Button>
                  </Flex>
      ...
      <Button w='50%' m="4.5rem" onClick={sendVotes}>Submit</Button>
    </>
  )
  ...
  const submittedProp = (
    <motion.div initial="pageInitial" animate="pageAnimate" variants={{
      ...
      <Flex alignItems="center">
        <Heading mt="10" mb="10" size="2xl" textAlign="center" mr="2rem">You voted!</Heading>
  ...
  const loginProp = (
    <>
      {submitted ? submittedProp : voteProp}
    </>
  )

  const logoutProp = (
    <>
      <Heading mt="10" mb="10" size="2xl" textAlign="center">Login to Vote!</Heading>
    </>
  )
  ...
  async function getInitiatives() {
    try {
      ...
      const response = await fetch(`${host}/initiatives?elect_id=${electId}`, req);
      if (response.status === 200) {
        const data = await response.json();
        var item = []
  ...
  async function checkVoted() {
    ...
    axios
      ...
      .catch(function (error) {
        setErrorMsg('An error has occurred, please refresh the page to try again.')
      })
  }

  return (
    <>
      {Banner({ title: "Vote", subtitle: "Elect the initiatives you desire", imgUrl: agvn })}
      <Flex m="5rem" flexDir="column" alignItems="center" justifyContent="center">
